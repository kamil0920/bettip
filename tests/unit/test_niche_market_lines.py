...
class TestSniperBetTypesSchema:
    ...
    def test_niche_variant_count(self):
        """76 niche variants: 12 shots + 14 fouls + 12 cards + 8 corners + 6 goals + 6 hgoals + 6 agoals + 6 cornershc + 2 cardshc + 4 ht."""
        from experiments.run_sniper_optimization import BET_TYPES
        niche = [n for n in BET_TYPES if "_over_" in n or "_under_" in n]
        assert len(niche) == 76, f"Expected 76 niche variants, got {len(niche)}: {sorted(niche)}"
