...
12. **TRACKING SIGNAL (TS) IS ASYMMETRIC — NEGATIVE TS IS SAFE.** TS measures cumulative directional bias. Negative TS = model underpredicts (says 80%, reality 90%) = conservative, you bet less than you could but still profit. Positive TS = model overpredicts (says 90%, reality 80%) = DANGEROUS, you oversize positions and lose money. **Never reject a model solely on negative TS when precision ≥ 70% and ECE < 0.05.** The current `max_ts=4.0` symmetric gate is too strict for high-base-rate niche markets (sot, dc_x2, etc.) where small calibration offsets accumulate into large |TS| on 50-200 bets. Per-fold TS within ±4 + good ECE + good precision = safe to deploy even if holdout |TS| > 10. Holdout TS explosion with normal per-fold TS indicates temporal concept drift at the fold boundary, not systematic model failure.
