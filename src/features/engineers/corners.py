...
class CornerFeatureEngineer(BaseFeatureEngineer):
    ...
    def __init__(
        ...
        Args:
            ...
            use_league_relative: Normalize features relative to league averages
        """
    ...
    def _build_team_features(self, df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
        ...
        for team in team_history["team"].unique():
            ...
            stats = {
                "team": team,
                "dates": pd.to_datetime(team_df["date"]).dt.tz_localize(None).values,
            }
            ...
            for col in ["corners_won", "corners_conceded", "shots", "shots_conceded"]:
                if col in team_df.columns:
                    ...
                    for w in self.window_sizes:
                        stats[f"{col}_roll_{w}"] = (
                            shifted.rolling(w, min_periods=self.min_matches).mean().values
                        )

                    # EMA
                    if self.use_ema:
                        stats[f"{col}_ema"] = (
                            shifted.ewm(span=self.ema_span, min_periods=self.min_matches)
                            .mean()
                            .values
                        )
    ...
    def _build_match_features(
        self, df: pd.DataFrame, team_stats: Dict[str, pd.DataFrame]
    ) -> pd.DataFrame:
        ...
        for idx, row in df.iterrows():
            ...
            if home_team in team_stats:
                ...
                if mask.any():
                    ...
                    # Core features
                    feat["home_corners_won_ema"] = latest.get(
                        "corners_won_ema", self.DEFAULTS["corners_won"]
                    )
                    feat["home_corners_conceded_ema"] = latest.get(
                        "corners_conceded_ema", self.DEFAULTS["corners_conceded"]
                    )
                    feat["home_shots_ema"] = latest.get("shots_ema", self.DEFAULTS["shots"])

                    # Rolling windows
                    for w in self.window_sizes:
                        feat[f"home_corners_won_roll_{w}"] = latest.get(
                            f"corners_won_roll_{w}", np.nan
                        )
                        feat[f"home_corners_conceded_roll_{w}"] = latest.get(
            ...
            if away_team in team_stats:
                ...
                if mask.any():
                    ...
                    feat["away_corners_won_ema"] = latest.get(
                        "corners_won_ema", self.DEFAULTS["corners_won"]
                    )
                    feat["away_corners_conceded_ema"] = latest.get(
                        "corners_conceded_ema", self.DEFAULTS["corners_conceded"]
                    )
                    feat["away_shots_ema"] = latest.get("shots_ema", self.DEFAULTS["shots"])

                    for w in self.window_sizes:
                        feat[f"away_corners_won_roll_{w}"] = latest.get(
                            f"corners_won_roll_{w}", np.nan
                        )
                        feat[f"away_corners_conceded_roll_{w}"] = latest.get(
                            f"corners_conceded_roll_{w}", np.nan
                        )
    ...
    def _add_derived_features(self, feat: Dict) -> None:
        ...
        feat["expected_total_corners"] = (
            feat["expected_home_corners"] + feat["expected_away_corners"]
        )
...
def build_corner_features(
    ...
    """
    engineer = CornerFeatureEngineer(window_sizes=window_sizes, min_matches=min_matches)
