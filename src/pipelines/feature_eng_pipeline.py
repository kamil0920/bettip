...
class FeatureEngineeringPipeline:
    ...
    def run(self, output_filename: str = "features.csv") -> pd.DataFrame:
        ...
        # NOTE: Rolling z-score normalization is applied per-market in regeneration.py
        # (only for niche markets where S12 A/B test showed it helps).
        # The pipeline stores raw features; normalization happens during sniper optimization.
    ...
    def _clean_data(self, raw_data: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
        ...
        if 'events' in cleaned_data:
            ...
            if not card_events.empty:
                ...
                # Do NOT fill card NaN→0. API-Football often returns match results
                # without card events, causing fake zeros that corrupt card models.
                # NaN cards propagate through EMAs and targets, which is correct
