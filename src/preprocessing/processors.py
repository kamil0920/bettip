...
class SeasonDataProcessor:
    ...
    def __init__(
        ...
        Args:
            config: Processing configuration (from YAML)
    ...
    def process_all_seasons(self) -> Dict[str, pd.DataFrame]:
        ...
        for season in self.config.seasons:
            try:
                ...
                if self.config.preprocessing.error_handling == "raise":
                    raise
    ...
    def process_season(self, season: int) -> Dict[str, pd.DataFrame]:
        ...
        season_dir = self.config.get_raw_season_dir(season)

        fixtures = self.fixtures_loader.load_fixtures(season_dir)
        if not fixtures:
            self.logger.warning(f"No fixtures found for season {season}")
            return {}
        ...
        for fixture in tqdm(fixtures, desc=f"Season {season}"):
            ...
            if self.config.preprocessing.include_events:
            ...
            if self.config.preprocessing.include_player_features:
    ...
    def _save_season_data(
        ...
        output_dir = self.config.get_preprocessed_season_dir(season)
