...
def save_config(results_df: pd.DataFrame, output_path: str) -> None:
    """Save forecastability results as JSON config for production use.
    ...
    config = {
        "description": (
            "Static forecastability metrics from permutation entropy analysis. "
            f"Generated by: python experiments/forecastability_analysis.py --output-json {output_path}"
        ),
        "markets": {},
    }
    ...
    for _, row in results_df.iterrows():
        ...
        config["markets"][market] = {
            "mean_pe_residual": round(float(row["mean_pe_residual"]), 4)
            if not np.isnan(row["mean_pe_residual"])
            else None,
            "pi_max": round(float(row["mean_pi_max"]), 4)
            if not np.isnan(row["mean_pi_max"])
            else None,
    ...
    logger.info(f"Forecastability config saved to {output_path}")
...
def main():
    ...
    parser.add_argument(
        "--output-json",
        default=None,
        help="Save forecastability config as JSON (e.g., config/forecastability.json)",
    )
    ...
    if args.output_json:
        save_config(results_df, args.output_json)
        logger.info(f"JSON config saved to {args.output_json}")
