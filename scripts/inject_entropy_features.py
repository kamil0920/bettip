#!/usr/bin/env python3
"""Inject EntropyFeatureEngineer outputs into existing features parquet.

Avoids full regenerate_all_features.py (~30 min) by bolting new columns onto existing file.
Same pattern as inject_window_ratios.py.
"""
import sys
from pathlib import Path

import numpy as np
import pandas as pd

# Ensure project root on path
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))

from src.features.engineers.entropy import EntropyFeatureEngineer

FEATURES_PATH = Path("data/03-features/features_all_5leagues_with_odds.parquet")

# Suffixes that identify entropy features
ENTROPY_SUFFIXES = ('_pe', '_sampen', '_pe_diff', '_pe_sum', '_sampen_diff', '_sampen_sum')


def inject_entropy_features(df: pd.DataFrame) -> pd.DataFrame:
    """Compute and merge entropy features into existing dataframe."""
    eng = EntropyFeatureEngineer()

    # Load match stats and build features directly
    match_stats = eng._load_match_stats()
    if match_stats.empty:
        raise RuntimeError("No match_stats loaded — check data/01-raw/")

    match_stats = eng._derive_cards(match_stats)
    featured = eng._build_features(match_stats)

    # Extract only entropy feature columns + fixture_id
    entropy_cols = [
        c for c in featured.columns
        if any(c.endswith(s) for s in ENTROPY_SUFFIXES)
    ]
    print(f"  Entropy features generated: {len(entropy_cols)}")

    entropy_df = featured[['fixture_id'] + entropy_cols].copy()

    # Drop any existing entropy columns to avoid _x/_y
    existing = [c for c in df.columns if c in entropy_cols]
    if existing:
        print(f"  Dropping {len(existing)} existing entropy cols to prevent collision")
        df = df.drop(columns=existing)

    merged = df.merge(entropy_df, on='fixture_id', how='left')
    return merged


def validate(df: pd.DataFrame, original_rows: int) -> None:
    """Validate the enhanced dataframe."""
    entropy_cols = [
        c for c in df.columns
        if any(c.endswith(s) for s in ENTROPY_SUFFIXES)
    ]

    assert len(df) == original_rows, f"Row mismatch: {len(df)} vs {original_rows}"

    print(f"\n--- Validation ---")
    print(f"  Rows: {len(df)}, Cols: {len(df.columns)}")
    print(f"  Entropy features: {len(entropy_cols)}")
    for c in sorted(entropy_cols):
        print(f"    {c}")

    # No _x/_y
    xy_cols = [c for c in df.columns if c.endswith('_x') or c.endswith('_y')]
    assert len(xy_cols) == 0, f"_x/_y collision: {xy_cols}"
    print(f"  _x/_y columns: 0")

    # No inf
    inf_count = df[entropy_cols].apply(
        lambda x: x.isin([float('inf'), float('-inf')]).sum()
    ).sum()
    assert inf_count == 0, f"Inf values found: {inf_count}"
    print(f"  Inf values: 0")

    # Coverage
    coverage = df[entropy_cols].notna().mean()
    print(f"  Coverage per feature:")
    for c in sorted(entropy_cols):
        print(f"    {c}: {coverage[c]*100:.1f}%")
    print(f"  Avg coverage: {coverage.mean()*100:.1f}%")


def main():
    print(f"Loading {FEATURES_PATH}...")
    df = pd.read_parquet(FEATURES_PATH)
    original_rows = len(df)
    print(f"  Shape: {df.shape}")

    print("\nInjecting entropy features...")
    df = inject_entropy_features(df)

    validate(df, original_rows)

    print(f"\nSaving to {FEATURES_PATH}...")
    df.to_parquet(FEATURES_PATH, index=False)
    print("Done.")


if __name__ == '__main__':
    main()
