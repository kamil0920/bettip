...
class FootballDataManager:
    ...
    def collect_player_statistics(self, league_key: str, season: int,
                                  max_fixtures: int = None, completed_only: bool = True) -> bool:
        ...
        try:
            ...
            if len(fixtures) == 0:
                logger.warning(f"⚠️  No completed fixtures found for player statistics")
                return False
            ...
            for i, fixture in enumerate(fixtures, 1):
                ...
                if players_file.exists():
                    logger.info(f"⏩ {i}/{len(fixtures)}: Player stats for fixture {fixture_id} already collected")
                    collected_count += 1
                    continue
                ...
                try:
                    ...
                    if players_data and len(players_data) > 0:
                        ...
                        logger.warning(f"   ⚠️  No player statistics for fixture {fixture_id}")
                        failed_count += 1
            ...
            logger.info(f"📊 Player statistics collection summary:")
            logger.info(f"   ✅ Successfully collected: {collected_count}")
            logger.info(f"   ❌ Failed: {failed_count}")
            logger.info(f"   📊 Success rate: {collected_count / (collected_count + failed_count) * 100:.1f}%" if (
                                                                                                                         collected_count + failed_count) > 0 else "N/A")
...
def bulk_collect(league_key: str = 'premier_league',
...
def main():
    """Main entry point with player statistics support."""
    ...
    parser.add_argument('--include-player-stats', action='store_true',
                        help='Include player statistics (rating, shots, passes, etc.)')
    ...
    if args.mode == 'bulk':
        ...
        if args.include_player_stats:
            manager.collect_player_statistics(args.league, args.season, args.max_fixtures)
